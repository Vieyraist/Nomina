...
def empleados_view():
    ...
    if request.method == "POST":
        ...
        for emp in empleados:
            ...
            zona = request.form.get(f"zona_{emp['id']}", emp["zona"])
            puesto = request.form.get(f"puesto_{emp['id']}", emp["puesto"])
            nombre = request.form.get(f"nombre_{emp['id']}", emp["nombre"])
            salario = to_float(request.form.get(f"salario_{emp['id']}", emp["salario"]))
            tarifa = to_float(request.form.get(f"tarifa_{emp['id']}", emp["tarifa"]))
            fecha_ingreso = request.form.get(f"fecha_{emp['id']}", emp["fecha_ingreso"])
            ...
            if not fecha_ingreso:
                ...
                try:
                    ...
                    fecha_ingreso = fecha_obj.strftime("%Y-%m-%d")
                except:
                    fecha_ingreso = emp["fecha_ingreso"]
            ...
            if ENV == "dev":
                cur.execute("""
                    UPDATE empleados
                    SET zona=?, puesto=?, nombre=?, salario=?, tarifa=?, fecha_ingreso=?
                    WHERE id=?
                """, (zona, puesto, nombre, salario, tarifa, fecha_ingreso, emp["id"]))
            else:
                cur.execute("""
                    UPDATE empleados
                    SET zona=%s, puesto=%s, nombre=%s, salario=%s, tarifa=%s, fecha_ingreso=%s
                    WHERE id=%s
                """, (zona, puesto, nombre, salario, tarifa, fecha_ingreso, emp["id"]))
